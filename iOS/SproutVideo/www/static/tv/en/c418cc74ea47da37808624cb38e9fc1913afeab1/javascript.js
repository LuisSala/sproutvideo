...
Tv.scheduleController.set("content",Tv.store.find(Tv.Schedule));Tv.mainPane=SC.MainPane.design({childViews:"videoPage".w(),videoPage:SC.ScrollView.extend({layout:{top:0,bottom:0,left:0,right:0},hasHorizontalScroller:NO,contentView:SC.View.extend(SC.StaticLayout,{useStaticLayout:YES,layout:{height:"auto"},childViews:["list"],list:Tv.VideoTemplateView})})});
...
this.invokeLater(function(){$f(id,"http://sala.us/flowplayer/flowplayer-3.2.7.swf",{clip:{autoPlay:true,autoBuffering:true}}).ipad({simulateiDevice:false})
},0)},render:function(context){arguments.callee.base.apply(this,arguments);context.attr("href",this.get("videoUrl"));
context.attr("style","background-image:url("+this.get("previewUrl")+")");context.begin("img").attr("src","/static/tv/en/c418cc74ea47da37808624cb38e9fc1913afeab1/source/resources/images/play_large.png").end()
